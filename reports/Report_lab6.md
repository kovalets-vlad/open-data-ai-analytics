...
### 5.1. Стан застосунку в Argo CD

На скріншоті панелі керування Argo CD відображено успішну синхронізацію («Synced») та здоровий стан («Healthy») усіх об'єктів кластера. Це підтверджує, що Argo CD коректно зчитав маніфести з GitHub-репозиторію та розгорнув їх у неймспейсі `ai-analytics`. У дереві ресурсів чітко видно взаємозв'язок між `Deployment`, `ReplicaSet` та запущеними `Pods`.

> ![](img/argo-web.png)

### 5.2. Функціонування веб-застосунку
...
### 5.3. Перевірка GitOps-циклу та масштабування
